...
-- 1. Drop existing function that needs signature change
DROP FUNCTION IF EXISTS public.get_profile_financial_data(UUID);

-- 2. Remove the unencrypted sensitive columns from profiles table
...
-- 3. Update the masked financial function to only use encrypted indicators
...
-- 4. Create new financial data access function that only returns encrypted versions
...
-- 5. Enhanced audit trigger to only track encrypted fields
...
-- 6. Grant permissions for the updated function
