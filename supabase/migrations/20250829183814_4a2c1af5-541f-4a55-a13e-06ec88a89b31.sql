-- Comprehensive Security Enhancement for Clients Table (Fixed)
...
-- More detailed audit trail for compliance and security monitoring (INSERT/UPDATE/DELETE only)
...
-- 4. CREATE TRIGGERS FOR ENHANCED AUDIT LOGGING (INSERT/UPDATE/DELETE only)
...
-- 5. CREATE ADDITIONAL RLS POLICY WITH RATE LIMITING
-- This policy ensures rate limiting is applied to client data access
DROP POLICY IF EXISTS "clients_rate_limited_access" ON public.clients;
...
-- 8. CREATE VIEW FOR SAFE CLIENT DATA ACCESS
-- This view automatically applies appropriate security levels
...
SELECT 
  ...
  c.updated_at,
  c.org_id,
  -- Sensitive fields are masked by default in this view
  'MASKED' as email_status,
  'MASKED' as phone_status,
  'MASKED' as tax_number_status,
  'MASKED' as vat_id_status,
  'MASKED' as notes_status
FROM public.clients c
...
GRANT SELECT ON public.clients_safe_view TO authenticated;